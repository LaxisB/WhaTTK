...
export const StoreContext = createContext<{ store: typeof store; actions: Actions }>();
...
export function runSimulation() {
    ...
    for (const scenario of Object.values(store.scenarios)) {
        ...
        const result = simulate(scenario.weapon, scenario.target, {
            label: scenario.name,
            accuracy: scenario.props.accuracy! / 100,
            headshotRate: scenario.props.headshotRate! / 100,
            runs: scenario.props.runs,
        });
        results.push(result);
    }
